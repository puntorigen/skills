#!/usr/bin/env python3
"""
object-to-3d step 2 - recover camera poses + a sparse point cloud with COLMAP.
...
Matching strategy (pick with --matcher):
  - exhaustive (default for objects): match every pair. An orbit is a loop of
    unordered close-up views, so any-to-any pairing is the robust choice.
    O(n^2) but practical to ~400 frames.
  - sequential: match each frame to its temporal neighbors. Faster; use for a
    single slow continuous sweep rather than a multi-pass orbit.
...
def main(argv=None):
    ...
    p.add_argument("--matcher", choices=["sequential", "exhaustive"], default="exhaustive",
                   help="feature matcher (default exhaustive; best for object orbits)")
    ...
    p.add_argument("--min-registered", type=float, default=0.9, dest="min_registered",
                   help="warn if fewer than this fraction of frames register "
                        "(default 0.9; clean object orbits should register almost all)")
