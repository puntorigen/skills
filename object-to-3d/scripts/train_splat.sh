...
#   --export-every N    checkpoint export cadence (default 1000; headless Brush
#                       prints NOTHING, so the splat.ply mtime advancing is the
#                       only progress signal - and a killed run keeps the last
#                       checkpoint)
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --export-every)   EXPORT_EVERY="$2"; shift 2 ;;
...
# checkpoint at most every EXPORT_EVERY steps: headless Brush is silent, so the
# refreshing splat.ply mtime is the progress bar (each export overwrites it)
[[ "$EXPORT_EVERY" -gt "$STEPS" ]] && EXPORT_EVERY="$STEPS"
CMD=("$BRUSH_BIN" "$SRC"
     --total-steps "$STEPS"
     --sh-degree "$SH_DEGREE"
...
echo "[train] progress   : headless Brush prints nothing; watch $PLY_OUT" >&2
echo "[train]              appear/refresh every $EXPORT_EVERY steps (ls -la it)" >&2
"${CMD[@]}"
