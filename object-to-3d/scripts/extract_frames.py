#!/usr/bin/env python3
"""
object-to-3d step 1 - turn an mp4 of an orbited object into frames for SfM.

Same frame selection as video-to-splat, with object-oriented defaults (--fps 3,
--max-frames 150): an orbit revisits the object from many close angles, so a
denser selection captures the whole surface while objects stay sharp.

Structure-from-Motion (COLMAP) and Gaussian-splat training want sharp, well
spread, non-duplicate views with plenty of overlap. Phone orbits are full
...
def main(argv=None):
    ...
    p.add_argument("--fps", type=float, default=3.0,
                   help="target selected frames per second (default 3 for objects)")
    p.add_argument("--oversample", type=int, default=3,
                   help="candidate extraction fps multiplier for blur rejection (default 3)")
    p.add_argument("--max-frames", type=int, default=150, dest="max_frames",
                   help="cap on selected frames; COLMAP time grows fast (default 150)")
